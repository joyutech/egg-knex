...
class BaseDao {
  ...
  async autoCreateTable() {
    ...
    // 字段
    Object.keys(tableStruct.column).forEach(function(name) {
      items.push(`${name} ${tableStruct.column[name]}`);
    });
    // 索引
    Object.keys(tableStruct.index).forEach(function(name) {
      let { type, key, using } = tableStruct.index[name];
      if(_.isArray(key)) key = key.join(',');
  ...
  async upsert(examples, duplicateKeys, updateKeys) {
    return await this.baseUpsert(this.knex, examples, duplicateKeys, updateKeys);
  }
}
