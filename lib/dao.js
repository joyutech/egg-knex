...
class BaseDao {
  ...
  async autoCreateTable() {
    ...
    if(!tableStruct || !tableStruct.column) return;

    let items = [];
    let hasPrimary = false;// 是否有主键
    // 字段
    Object.keys(tableStruct.column).forEach(function(key) {
      items.push(`${key} ${tableStruct.column[key]}`);
    });
    // 索引
    Object.keys(tableStruct.index).forEach(function(key) {
      ...
      if(type === 'primary') {
        hasPrimary = true;
        items.push(`primary key (\`${key}\`) using ${tableStruct.index[key].using}`);
      } else if(type==='unique') {
        items.push(`unique key \`${tableStruct.index[key].name}\` (\`${key}\`) using ${tableStruct.index[key].using}`);
      } else if(type==='normal') {
        items.push(`key \`${tableStruct.index[key].name}\` (\`${key}\`) using ${tableStruct.index[key].using}`);
      }
    });
    const createSql = `create table ${this.table} (${items.join(',')}) ${tableStruct.option};`;
    if(!hasPrimary) {
      // 没有主键的话，报个警告
      app.knexLogger.warn(`[egg-knex] auto create ${this.table} table are no primary key config`);
    }
    try {
      await this.knex.raw(createSql);
      app.knexLogger.warn(`[egg-knex] auto create ${this.table} table success: ${createSql}`);
    } catch(err) {
      if(err.code === 'ER_TABLE_EXISTS_ERROR')
        app.knexLogger.warn(`[egg-knex] auto create ${this.table} table failed: table is exists`);
      else
        app.knexLogger.error(`[egg-knex] auto create ${this.table} table failed: ${err}`);
    }
  }
...
async function loadDao(app, client, config) {
  if (!config.directory) return resolve();
